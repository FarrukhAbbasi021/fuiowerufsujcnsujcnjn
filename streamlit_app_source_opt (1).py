...
#qw- for summarization with prompt template
        ...
        for source in source_list:
            ...
            if answer_type == "Projects with sources only":
                ...
                retriever = vectorstore.as_retriever(
                    search_kwargs={"k": 10, 'filter': {'source': source}})
            ...
            html = html + \
                f'<button type="button" class="accordion-button collapsed" data-bs-toggle="collapse" data-bs-target="#collapse-{source_id}">CHILD Project Source {source_id}</button>'
            html = html + '</h2>'
            html = html + \
